...
export const bookEventHandlerMap: BookEventHandlerMap<BookEvent, BookEventContext> = {
	...
	// customised -- Portal Wild: portal symbol(s) + 1-3 neighbours become Wild before wins
	// are evaluated for this step. The event carries the post-conversion board, so we just
	// settle straight to it and briefly highlight the converted cells.
	portalWildInfo: async (bookEvent: BookEventOfType<'portalWildInfo'>) => {
		stateGameDerived.enhancedBoard.settle(bookEvent.board);
		const positions = _.flatten(bookEvent.conversions.map((c) => c.converted));
		await animateSymbols({ positions });
	},
	// customised -- Orb Collector: every visible Orb multiplier value is summed into the
	// persistent Global Multiplier, then Orb+Collector symbols are removed on the next
	// tumble. The event carries the board *before* that removal, so settle to it, then
	// highlight the orbs/collector and push the updated Global Multiplier.
	orbCollectorInfo: async (bookEvent: BookEventOfType<'orbCollectorInfo'>) => {
		stateGameDerived.enhancedBoard.settle(bookEvent.board);
		await animateSymbols({ positions: [...bookEvent.orbPositions, ...bookEvent.collectorPositions] });
		eventEmitter.broadcast({ type: 'tumbleWinAmountShow' });
		eventEmitter.broadcast({
			type: 'tumbleWinAmountUpdate',
			amount: bookEvent.collectedTotal,
			animate: false,
		});
		eventEmitter.broadcast({ type: 'globalMultiplierShow' });
		await eventEmitter.broadcastAsync({
