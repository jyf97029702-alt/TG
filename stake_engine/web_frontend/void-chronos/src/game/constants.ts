...
// initial board (padded top and bottom): 5 reels x 6 visible rows (+1 padding row each side)
export const INITIAL_BOARD: RawSymbol[][] = Array.from({ length: 5 }, (_, reelIndex) => {
	const cycle: RawSymbol['name'][] = ['S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8'];
	return Array.from({ length: 8 }, (_, rowIndex) => ({
		name: cycle[(reelIndex + rowIndex) % cycle.length],
	}));
});
...
export const HIGH_SYMBOLS = ['S5', 'S6', 'S7', 'S8'];
