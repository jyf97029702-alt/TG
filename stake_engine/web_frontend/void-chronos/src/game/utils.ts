...
export const getSymbolKey = ({ rawSymbol }: { rawSymbol: RawSymbol }) => {
	if (rawSymbol.multiplier !== undefined) {
		return `${rawSymbol.name}_${rawSymbol.multiplier}`;
	}
	return rawSymbol.name;
};

// VOID CHRONOS renders every symbol procedurally (see SymbolGraphic.svelte) instead of
// from Spine/sprite-sheet assets, so this no longer needs an asset lookup table -- it
// just needs to not throw. `type` is checked in a couple of places (e.g. ReelSymbol's
// `animating` gate) purely to special-case Spine symbols; 'procedural' safely falls
// through those checks as "not spine".
export const getSymbolInfo = (_args: { rawSymbol: RawSymbol; state: SymbolState }) => {
	return { type: 'procedural' as const };
};
